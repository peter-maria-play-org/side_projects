...
class Task(BaseModel):
    ...
    def pretty_print(self, current_time: datetime=datetime.now(), base_indent:int=0):
        """
        Pretty printing for the task.
        """

        render_str = indent("Task:\n",base_indent*'\t')
        render_str += indent(f"Name: {self.name}\n", (base_indent+1)*'\t')
        render_str += indent(f"Description: {self.description}\n", (base_indent+1)*'\t')
        render_str += indent(f"Creation Time: {self.creation_time}\n", (base_indent+1)*'\t')
        render_str += indent(f"Deadline: {self.deadline}\n", (base_indent+1)*'\t')
        render_str += indent(f"Priority: {self.priority.name}\n", (base_indent+1)*'\t')
        render_str += indent(f"Score: {self.compute_score(current_time)}", (base_indent+1)*'\t')
        print(render_str)
...
class TaskMaster(BaseModel):
    ...
    def pretty_print(self, current_time: datetime=datetime.now()):
        """
        Pretty printing for the TaskMaster.
        TODO add base_indent
        """
        ...
        for task in self.task_list:
            task.pretty_print(base_indent=1, current_time=current_time)
